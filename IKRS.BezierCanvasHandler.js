...
IKRS.BezierCanvasHandler = function() {
    ...
    this.undoHistory = new IKRS.UndoHistory( this.bezierPath, 32);
    ...
    this.loadBackgroundImage( _DILDO_CONFIG.IMAGES.BEZIER_BACKGROUND, true /* redraw when ready */); 
...
IKRS.BezierCanvasHandler.prototype.acquireOptimalView = function( frameSize ) {
  ...
  var bounds        = this.getBezierPath().computeBoundingBox();
  var moveAmount    = new THREE.Vector2( bounds.getWidth() / 2.0 - bounds.xMax, bounds.getHeight() / 2.0 - bounds.yMax);
...
IKRS.BezierCanvasHandler.prototype._drawAnonymousBackgroundImage = function( image, keepRatio ) {
  ...
  this.context.drawImage( image, (contextWidth - drawWidth)/2, (contextHeight - drawHeight)/2, 
    drawWidth, // 512,
    drawHeight // 768
  );
};
...
IKRS.BezierCanvasHandler.prototype._drawWithoutBackgroundImages = function() {
  ...
  if( document.forms["bezier_form"] &&
    ...
    this._drawRulers( this.context, this.drawOffset, this.zoomFactor, boundingBox);
  }
  ...
  if( document.forms["bezier_form"].elements["draw_bounding_box"].checked ) {
    ...
    this.context.beginPath();
    this.context.moveTo( rightUpperPoint.x * this.zoomFactor + this.drawOffset.x, rightUpperPoint.y * this.zoomFactor + this.drawOffset.y);
    this.context.lineTo( rightLowerPoint.x * this.zoomFactor + this.drawOffset.x, rightLowerPoint.y * this.zoomFactor + this.drawOffset.y);
    ...
    for( var c = 0; c < this.getBezierPath().getCurveCount(); c++ ) {
      ...
      for( var s = 0; s < curve.segmentCache.length; s++ ) {
        ...
        this.context.lineTo( segment.x * this.zoomFactor + this.drawOffset.x, segment.y * this.zoomFactor + this.drawOffset.y);
      }
    }
  ...
  if( document.forms["bezier_form"].elements["draw_bounding_box"].checked ) {
    ...
    this.context.beginPath();
    this.context.moveTo( leftUpperPoint.x * this.zoomFactor + this.drawOffset.x + lineDistance, leftUpperPoint.y * this.zoomFactor + this.drawOffset.y + lineDistance + 10);
    this.context.lineTo( leftUpperPoint.x * this.zoomFactor + this.drawOffset.x + lineDistance, leftUpperPoint.y * this.zoomFactor + this.drawOffset.y + lineDistance );
    this.context.lineTo( leftUpperPoint.x * this.zoomFactor + this.drawOffset.x + lineDistance + 10, leftUpperPoint.y * this.zoomFactor + this.drawOffset.y + lineDistance );
    this.context.stroke();
...
IKRS.BezierCanvasHandler.prototype._drawRulers = function( context, drawOffset, zoomFactor, bounds) {
...
IKRS.BezierCanvasHandler.drawVerticalRuler = function( context, drawOffset, zoomFactor, bounds, distance, markerLength, millimeterPerPixel, markerDistance, textDistance) {
  ...
  // Draw vertical ruler
  context.beginPath();
  context.moveTo( rightLowerPoint.x * zoomFactor + drawOffset.x + distance, rightLowerPoint.y * zoomFactor + drawOffset.y);
  context.lineTo( rightUpperPoint.x * zoomFactor + drawOffset.x + distance, rightUpperPoint.y * zoomFactor + drawOffset.y);
  context.stroke();
  ...
  while( (relY = i * markerDistance) < height ) {
    ...
    context.lineTo( x + distance + markerLength, y );	
    context.stroke();

    // Draw unit number
    var mm = relY * millimeterPerPixel;
    context.fillText( "" + mm + "mm", x + distance + markerLength + textDistance, y );

    i++;
  }
}

IKRS.BezierCanvasHandler.drawHorizontalRuler = function( context, drawOffset, zoomFactor, bounds, distance, markerLength, millimeterPerPixel, markerDistance, textDistance) {
  ...
  // Draw vertical ruler
  context.beginPath();
  context.moveTo( rightLowerPoint.x * zoomFactor + drawOffset.x, rightLowerPoint.y * zoomFactor + drawOffset.y + distance);
  context.lineTo( leftLowerPoint.x * zoomFactor + drawOffset.x, leftLowerPoint.y * zoomFactor + drawOffset.y + distance);
  context.stroke();
  ...
  while( (relX = i*markerDistance) < width ) {
    ...
    context.beginPath();
    context.moveTo(x, y + distance);
    context.lineTo(x, y + distance + markerLength);	
    context.stroke();

    // Draw unit number
    var mm = relX * millimeterPerPixel;
    context.rotate(Math.PI/2);
    context.fillText("" + mm + "mm", y + distance + markerLength + textDistance, -x );
    context.rotate(-Math.PI / 2);
...
IKRS.BezierCanvasHandler.prototype._drawSelectedPoint = function( context, point, zoomFactor, drawOffset) {
...
IKRS.BezierCanvasHandler.prototype.drawBezierCurve = function( context, bezierCurve, drawOffset, zoomFactor, drawStartPoint, drawEndPoint, drawStartControlPoint, drawEndControlPoint, drawTangents, startPointIsSelected, endPointIsSelected) {
  ...
  if( drawTangents ) {
    ...
    context.beginPath();
    context.moveTo( bezierCurve.getStartPoint().x * zoomFactor + drawOffset.x, bezierCurve.getStartPoint().y * zoomFactor + drawOffset.y);
    context.lineTo( bezierCurve.getStartControlPoint().x * zoomFactor + drawOffset.x, bezierCurve.getStartControlPoint().y * zoomFactor + drawOffset.y);
    context.stroke();

    // Draw end point tangent
    context.beginPath();
    context.moveTo( bezierCurve.getEndPoint().x * zoomFactor + drawOffset.x, bezierCurve.getEndPoint().y * zoomFactor + drawOffset.y);
    context.lineTo( bezierCurve.getEndControlPoint().x * zoomFactor + drawOffset.x, bezierCurve.getEndControlPoint().y * zoomFactor + drawOffset.y );
    context.stroke();
  ...
  context.lineWidth   = 2;
  context.beginPath();
  context.moveTo( bezierCurve.segmentCache[0].x * zoomFactor + drawOffset.x, bezierCurve.segmentCache[0].y * zoomFactor + drawOffset.y);
  for( var i = 1; i < bezierCurve.segmentCache.length; i++ ) {
    context.lineWidth = 2;
    context.lineTo( bezierCurve.segmentCache[i].x * zoomFactor + drawOffset.x, bezierCurve.segmentCache[i].y * zoomFactor + drawOffset.y);	
  }
  ...
  if( drawStartPoint || drawEndPoint ) {
    ...
    if( drawEndPoint ) {
      ...
      if( endPointIsSelected ) {
        ...
        this._drawSelectedPoint( context, bezierCurve.getEndPoint(), zoomFactor, drawOffset );
      } else {
        context.fillStyle   = "#B400FF";
        context.strokeStyle = "#B400FF";
        context.fillRect( bezierCurve.getEndPoint().x * zoomFactor - 2 + drawOffset.x, bezierCurve.getEndPoint().y * zoomFactor - 2 + drawOffset.y, 5, 5 );
      }
    }
  ...
  if((drawStartControlPoint || drawEndControlPoint) ) {
    ...
    if( drawStartControlPoint ) {
      context.fillRect(bezierCurve.getStartControlPoint().x * zoomFactor - 2 + drawOffset.x, bezierCurve.getStartControlPoint().y * zoomFactor - 2 + drawOffset.y, 5, 5 );
    }
    // End control point?
    if( drawEndControlPoint ) {
      context.fillRect( bezierCurve.getEndControlPoint().x * zoomFactor - 2 + drawOffset.x, bezierCurve.getEndControlPoint().y * zoomFactor - 2 + drawOffset.y, 5, 5 );
    }
  }
}


IKRS.BezierCanvasHandler.prototype.drawPerpendiculars = function( context, bezierCurve, drawOffset, zoomFactor ) {
  ...
  while( i * pDistance <= bezierCurve.getLength() ) {
    ...
    context.beginPath();
    context.moveTo( point.x * zoomFactor + drawOffset.x, point.y * zoomFactor + drawOffset.y );
    context.lineTo( point.x * zoomFactor + drawOffset.x + perpendicular.x * (perpendicularLength*zoomFactor), point.y * zoomFactor + drawOffset.y + perpendicular.y * (perpendicularLength*zoomFactor));
    context.stroke();
...
IKRS.BezierCanvasHandler.prototype.locateCachedBezierPointNearPosition = function( point, tolerance ) { 
  for( var c = 0; c < this.getBezierPath().getCurveCount(); c++ ) {
    ...
    for( var s = 0; s < bCurve.segmentCache.length; s++ ) {
      ...
      if( this.pointIsNearPosition( tmpPoint, point.x, point.y, tolerance ) ) { 
        return [ c, s ];
      }
    }
  }
...
IKRS.BezierCanvasHandler.prototype.pointIsNearPosition = function( point, x, y, tolerance ) {
...
IKRS.BezierCanvasHandler.prototype.translateMouseEventToRelativePosition = function( parent, e ) {
...
IKRS.BezierCanvasHandler.prototype.mouseUpHandler = function( e ) {
  ...
  if( this.bezierCanvasHandler.latestClickTime && (currentTime - this.bezierCanvasHandler.latestClickTime) < 300 ) {
    this.bezierCanvasHandler.doubleClickHandler( this, e );
  } else if( this.bezierCanvasHandler.latestClickTime && (currentTime-this.bezierCanvasHandler.latestMouseDownTime) < 300 ) {
...
IKRS.BezierCanvasHandler.prototype.mouseMoveHandler = function( e ) {
  ...
  if( this.bezierCanvasHandler.latestMouseDownPosition ) {
    ...
    if( this.bezierCanvasHandler.currentDragPoint ) {
      ...
      this.bezierCanvasHandler.getBezierPath().moveCurvePoint(
        this.bezierCanvasHandler.draggedCurveIndex,
...
IKRS.BezierCanvasHandler.prototype.scaleBezierPathUniform = function( scaleFactor ) {
  this.getBezierPath().scale( new THREE.Vector2(0,0), new THREE.Vector2(scaleFactor,scaleFactor));
  this.redraw();
  this._fireChangeEvent( { nextEventFollowing: false  /* nextEventFollowing */ } );
};

IKRS.BezierCanvasHandler.prototype._scaleBezierPath = function( oldBounds, newBounds, anchor, redraw, nextEventFollowing) {

  // Calculate scale factors
  var scaling    = new THREE.Vector2( oldBounds.getWidth()  / newBounds.getWidth(), oldBounds.getHeight() / newBounds.getHeight());
  ...
  this.getBezierPath().scale(anchor, scaling);
...
IKRS.BezierCanvasHandler.prototype.doubleClickHandler = function( parent, e ) {
...
IKRS.BezierCanvasHandler.prototype.resolveBoundingBoxPointNear = function( point, tolerance) {
  var bounds = this.getBezierPath().computeBoundingBox();
  if( this.pointIsNearPosition( bounds.getLeftUpperPoint(), point.x, point.y, tolerance ) ) {
    return IKRS.BezierCanvasHandler.POINT_ID_LEFT_UPPER_BOUND;
  } else if( this.pointIsNearPosition( bounds.getRightUpperPoint(), point.x, point.y, tolerance ) ) {
    return IKRS.BezierCanvasHandler.POINT_ID_RIGHT_UPPER_BOUND;
  } else if( this.pointIsNearPosition( bounds.getRightLowerPoint(), point.x, point.y, tolerance ) ) {
    return IKRS.BezierCanvasHandler.POINT_ID_RIGHT_LOWER_BOUND;
  } else if( this.pointIsNearPosition( bounds.getLeftLowerPoint(), point.x, point.y, tolerance ) ) {
...
IKRS.BezierCanvasHandler.prototype.drawBezierPath = function(context, bezierPath, drawOffset, zoomFactor, drawStartPoint, drawEndPoint, drawStartControlPoint, drawEndControlPoint, drawTangents) {
  ...
  for( var i = 0; i < bezierPath.getCurveCount(); i++ ) {
    ...
    if( false ) { // document.forms["bezier_form"].elements["draw_linear_path_segments"].checked ) {
      ...
      context.lineWidth   = 1;
      context.beginPath();
      context.moveTo( bCurve.getStartPoint().x * zoomFactor + drawOffset.x, bCurve.getStartPoint().y * zoomFactor + drawOffset.y );
      context.lineTo( bCurve.getEndPoint().x   * zoomFactor + drawOffset.x, bCurve.getEndPoint().y   * zoomFactor + drawOffset.y );
      context.stroke();
    }
    ...
    if( document.forms["bezier_form"].elements["draw_perpendiculars"].checked ) {
      this.drawPerpendiculars(context, bCurve, drawOffset, zoomFactor);
    }
